...
**Note:** The application uses [dotenv](https://www.npmjs.com/package/dotenv) to load configuration
variables from a file on start up. If you would like environment variables set each time you run the
server then create a file called `.env` in the project root and configure your variables there.

**DO NOT** commit your `.env` file to source control!! The idea is to keep sensitive and per-environment
